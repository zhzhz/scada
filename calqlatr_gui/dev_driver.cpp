...
void Dev_driver::write_data(void *data)
{
    ...
    custom_data.l.name_variable = data_ex->name_variable;
    custom_data.l.name_variable_old = data_ex->name_variable_old;
...
//发送init时的读map用的，不使用data_save作为返回判断，而是使用新的独立的函数作为返回判断
void Dev_driver::write_data(QMap<QString, QMap<int, QMap<QString, QVariant>>> device_map, QStringList readList)
{
    QByteArray data_send;
    int ID = 3;
    QDataStream * stream = new QDataStream(&data_send, QIODevice::WriteOnly);
    (*stream) << ID << device_map << readList;//将custom_data序列化写入QByteArray
 ...
 * */
QDataStream& operator>>(QDataStream& src, data_exchange& data)
{
    /*
...
QDataStream& operator<<(QDataStream& out, data_exchange& data)
{
    //out << data.m_nType << data.m_strName;
    //out << data.ID;
    out << data.read_write << data.device << data.dev_id  << data.name_variable << data.write_data;
    return out;
}
